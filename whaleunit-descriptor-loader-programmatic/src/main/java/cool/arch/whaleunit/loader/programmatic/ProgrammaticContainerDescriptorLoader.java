...
/*
 * #%L WhaleUnit - Annotation Container Loader %% Copyright (C) 2015 CoolArch %% Licensed to
 * the Apache Software Foundation (ASF) under one or more contributor license agreements. See
...
public class ProgrammaticContainerDescriptorLoader implements
	ContainerDescriptorLoader<ProgrammaticContainerDescriptors> {
	...
	@Override
	public Collection<ContainerDescriptor> load(final ProgrammaticContainerDescriptors annotation)
		...
		return stream(sources).map(this::instantiate)
			.map(i -> i.get())
			.flatMap(c -> c.stream())
